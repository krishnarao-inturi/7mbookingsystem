...
;; multi select
;; ----------------------------------------------------
;; (def options [{ :value "one" :label "One" }
;;               { :value "two" :label "Two" }])

;; (defn my-multi-select []
;;   (let [data (r/atom [{:value "one", :label "One"}
;;                       {:value "two", :label "Two"}])]
;;     (fn []
;;       [:div
;;        [:p (str @data)]
...
;;          :disabled false
;;          :options options
;;          :on-change #(reset! data (js-clj %))}]])))

(defn multi-select [id placeholder value options disable?]
  [r-select
   {:name id
    :placeholder placeholder
    :value (or @value [])
    ...
    :options @options
    :on-change (fn [val]
                 (reset! value (mapv #(:id %) (js-clj val))))}])

(defn row-multiselect
  [label id placeholder value options disable? focus key val-fun ]
  [:div.form-group
   [:label.col-md-4.control-label label]
   [:div.col-md-8
    [multi-select id placeholder value options disable?]
    (if focus
      (if (= nil val-fun)
        [:div]
        [:div {:style  {:color "red"}}
   ...
   [:div.modal-dialog
    [:div.modal-content
     [:div.modal-header
      ...
      [:h4.modal-title "Confirmation Message"]]
     [:div.modal-body
      [:div.text-center
...
(declare check-all-routes)

(defn get-booking-agents-error [res]
  (case (:status res)
    401 (do (swap! session dissoc :session)
            (pushy/set-token! routes/history (routes/url-for :home)))
    (boot-notify
     "warning" "Warning :"
     "Problem with the sever, Please try again")))

(defn get-booking-agents [data]
  (ajax/GET
   (str server
        (if (check-all-routes "/fit" 4)
          "fitagents"
          "groupsagents"))
   {:handler #(reset! data %)
    :error-handler #(get-booking-agents-error %)
    :format :json
    :response-format :json
    :keywords? true
    :headers
...
(defn fit-group-topfields-comp [data-set state logos quotations]
  (let [role (get-in @session [:session :user :role])
        fit-or-group-agents (r/atom [])]
    (when-not (seq (@data-set :bookingagents))
      (swap! data-set assoc :bookingagents
             (case (get-in @session [:session :user :role])
               "bookadmin" []
               (vector (get-in @session [:session :user :id] 0)))))
    (get-booking-agents  fit-or-group-agents)
    (fn []
      [:div.row
       [:div.col-md-6
        ...
        [row-multiselect "7m Booking Agents" "agents" "Select Agents"
         (r/cursor data-set [:bookingagents]) fit-or-group-agents
         (if (= role "bookadmin") false true) (@state :focus)
         :bookingagents (bookings-validator @data-set)]
        ;; [:p (str (:bookingagents @data-set))]
    ...
    (loop [itineraries []
      ...
      (if (empty? coll)
        {:quotationid (js/parseInt (second quotation-info))
         ...
         :bookingagents (@data-set :bookingagents [])
...
(defn bookings-get-data-destruct-for-edit [data]
  (let [booking (get-in data [:booking 0])
    ...
    {:id (booking :id)
     ...
     :bookingagents (booking :bookingagents [])
...
(defn booking-generate-onclick-error [res state data-set]
  (case (:status res)
    401 (do (swap! session dissoc :session)
            (pushy/set-token! routes/history (routes/url-for :home)))
    (do
      (swap! state assoc :save-busy false)
...
(defn booking-generate-onclick [data-set state]
  (if (nil? (bookings-validator @data-set))
    (do
      (swap! state assoc :save-busy true)
          ...
          {:on-click
           #(do (reset! fitbookings-atom
                        {:bookingnumber "BK-00"
                         :isfinalbooking false
                         :emergencynumbers ""
                         :bookingagents (case (get-in @session [:session :user :role])
                                          "bookadmin" []
                                          (vector (get-in @session [:session :user :id] 0)))
                         :termsandconditions terms-and-conditions
                         :cities []
        ...
        [:center
         [:button.btn.btn-primary
          {:on-click #(booking-generate-onclick data-set state)
           :disabled (get @state :save-busy)}
          (str  (if (@data-set :id) "Save " "Generate ") "Booking ")
          (when (get @state :save-busy)
              ...
              [:td
               [:div.row
                [:div.col-md-8.col-md-offset-2.btn-group
                 ...
                 [anchor-tooltip
                  {:class "btn btn-info"
                   :href (routes/url-for :group-paymentlog :id (:id i))
                   :title "Payment Log"}
                  [:i.fa.fa-credit-card]]]]]
              [:td (:bookingnumber i)]
              [:td (:qrnumber i)]
...
;;     (do
;;       (swap! state assoc :focus false)
;;       (boot-notify "success" "Success :" "Problem with the server, Please try again"))))
          ...
          [:div.box-tools.pull-right
           [:div.btn-group
            ...
            [:a.btn.btn-default.btn-sm
             ...
             [:i.fa.fa-file-excel-o] " Export Activities"]]])]
       [:div.box-body
        ...
        [:div.col-md-6
         [:center
          (when-not (= (routes/url-for :view-item :id (@data :id 0))
            ...
            [:button.btn.btn-primary
             {:on-click #(item-save-onclick data state)
              :disabled (get @state :busy)} "Save "
             (when (get @state :busy)
               [:i.fa.fa-spinner.fa-pulse])])"  "
          [:a.btn.btn-default
...
;; Hotels Comp
;; -----------------------------------------------------------
(defn matches [value re]
  ((complement empty?) (re-seq re value)))

(defn check-optional-email [email]
  (if (seq email)
    (v/email email)
    true))
...
(defn hotel-validator [data-set]
  (first
   (b/validate
    ...
    :contactpersonemail [[check-optional-email :message "Please enter valid email"]])))
            ...
            [:a.btn.btn-default.btn-sm
             {:id "Downloadcsv"
              :href (str server "excel/hotels.xlsx?token="
                         (get-in @session [:session :token]))}
             [:i.fa.fa-file-excel-o] " Export Hotels"]]])]
       [:div.box-body
...
(defn hotel-save-onclick-error [res state filename]
  (case (res :status)
    ...
    (do (reset! filename "")
        (swap! state assoc :focus false)
        (swap! state assoc :busy false)
        (boot-notify "warning" "Warning :" "Problem with the server, Please try again"))))
       ...
       [:div.box-footer.clearfix
        [:center
         (when-not (= (routes/url-for :view-hotel :id (@data :id 0))
                      (pushy/get-token routes/history))
           [:button.btn.btn-primary
            {:on-click #(upload-hotel-contactfile data state filename)
             :disabled (get @state :busy)}
            "Save " (when (get @state :busy)
                      [:i.fa.fa-spinner.fa-pulse])])"  "
         [:a.btn.btn-default
...
(defn restaurant-validator [data-set]
  (first
   (b/validate
    ...
    :email [[check-optional-email :message "Please enter valid email"]]
    )))
            ...
            [:a.btn.btn-default.btn-sm
             {:id "Downloadcsv"
              :href (str server "excel/restaurents.xlsx?token="
                         (get-in @session [:session :token]))}
             [:i.fa.fa-file-excel-o] " Export Restaurants"]]])]
       [:div.box-body
...
(defn restaurant-save-onclick-success [res data state]
  ...
  (boot-notify
   "success" "Success :"
   "Restaurant is successfully saved"))
       ...
       [:div.box-footer.clearfix
        [:center
         (when-not (= (routes/url-for :view-restaurant :id (@data :id 0))
                      (pushy/get-token routes/history))
           [:button.btn.btn-primary
            {:on-click #(restaurant-save-onclick data state)
             :disabled (get @state :busy)} "Save "
            (when (get @state :busy)
              [:i.fa.fa-spinner.fa-pulse])])" "
         [:a.btn.btn-default
            ...
            [:a.btn.btn-default.btn-sm
             {:id "Downloadcsv"
              :href (str server "excel/tourguides.xlsx?token="
                         (get-in @session [:session :token]))}
             [:i.fa.fa-file-excel-o] " Export Tour Guides"]]])]
       [:div.box-body
         ...
         [:div.row
          ...
          [:div.col-md-6
           ...
           [row-input "Email" "Email" :email
            "email" "Enter email" (r/cursor data [:email])
            (@state :focus) (tourguide-validator @data)]
           [row-text-area "Bank/Payment Info" "Bank/Payment Info"
            :bankorpaymentinfo 3 "Enter Bank/Payment Info"
            (r/cursor data [:bankorpaymentinfo]) (@state :focus)
            (tourguide-validator @data)]
       ...
       [:div.box-footer.clearfix
        [:center
         (when-not (= (routes/url-for :view-tourguide :id (@data :id 0))
                      (pushy/get-token routes/history))
           [:button.btn.btn-primary
            {:on-click #(tourguide-save-onclick data state)
             :disabled (get @state :busy)} "Save "
            (when (get @state :busy)
              [:i.fa.fa-spinner.fa-pulse])])"  "
         [:a.btn.btn-default
            ...
            [:a.btn.btn-default.btn-sm
             {:id "Downloadcsv"
              :href (str server "excel/transportation.xlsx?token="
                         (get-in @session [:session :token]))}
             [:i.fa.fa-file-excel-o] " Export Transportations"]]])]
       [:div.box-body
...
(defn transportation-save-onclick-success [res data state]
  ...
  (boot-notify
   "success" "Success :"
   "Transportation is successfully saved"))
       ...
       [:div.box-footer.clearfix
        [:center
         (when-not (= (routes/url-for :view-transportation :id (@data :id 0))
                      (pushy/get-token routes/history))
           [:button.btn.btn-primary
            {:on-click #(transportation-save-onclick data state)
             :disabled (get @state :busy)} "Save "
            (when (get @state :busy)
              [:i.fa.fa-spinner.fa-pulse])])"  "
         [:a.btn.btn-default
            ...
            [:a.btn.btn-default.btn-sm
             {:id "Downloadcsv"
              :href (str server "excel/events.xlsx?token="
                         (get-in @session [:session :token]))}
             [:i.fa.fa-file-excel-o] " Export Events"]]])]
       [:div.box-body
        ...
        [:div.col-md-6
         [:center
          (when-not (= (routes/url-for :view-event :id (@data :id 0))
                       (pushy/get-token routes/history))
            [:button.btn.btn-primary
             {:on-click #(event-save-onclick data state)
              :disabled (get @state :busy)} "Save "
             (when (get @state :busy)
               [:i.fa.fa-spinner.fa-pulse])])"  "
          [:a.btn.btn-default
...
(defn delete-logo-table-modal [id src-in list]
  [:div.modal.fade
   {:role "dialog" :id "removemodal"}
   [:div.modal-dialog
    [:div.modal-content
     ...
     [:div.modal-body
      [:div.text-center
       [:p.lead "Are you sure you want to remove this Logo ?"]]]
     [:div.modal-footer
      [:button.btn.btn-primary
       {:id (str @id "remove")
        :type "button"
        :on-click #(delete-logo-on-click id src-in list)} "Yes"]
      [:button.btn.btn-default
       {:type "button"
        :data-dismiss "modal"} "No"]]]]])
        ...
        (when (= role "bookadmin")
          [:div.box-tools.pull-right
           [:a.btn.btn-primary.btn-sm
            ...
            "Add New Logo"]])]
       [:div.box-body
        [delete-logo-table-modal remove-id src-in list]
        [:div.row
         [:div.col-md-6.col-md-offset-6
          [:div.col-md-12.col-sm-12.col-xs-12.input-group.form-group
           ...
           (when (seq (@data :filename))
             [:button.btn.btn-primary
              {:on-click #(.open js/window
                                 (str server "logosfilename/" (@data :filename)))}
              [:span.glyphicon.glyphicon-download-alt]])]]]]
       [:div.box-footer.clearfix
        [:center
         (when-not (= (routes/url-for :view-logo :id (@data :id 0))
                      (pushy/get-token routes/history))
           [:button.btn.btn-primary
            {:on-click #(upload-logo-contactfile data state filename)
             :disabled (get @state :busy)} "Save "
            (when (get @state :busy)
              [:i.fa.fa-spinner.fa-pulse])])"  "
         [:a.btn.btn-default
  ...
  (let [role (get-in @session [:session :user :role])]
    ...
    (case role
      "bookadmin" (pushy/set-token!
                   routes/history
                   (routes/url-for :fitbookings))
      "bookstandard" (pushy/set-token!
                      routes/history
                      (routes/url-for :fitbookings))
      "bookgroup" (pushy/set-token!
                   routes/history
                   (routes/url-for :groupbookings))
...
(defn login-on-click [e data focus server-error]
  (do
    (.preventDefault e)
...
(defn row-register-select [label id options data-set focus val]
  [:div.form-group
   [:label.col-md-4.control-label label]
   [:div.col-md-8 [reg-select id options val]
    (if focus
      (if (= nil (register-validator @data-set))
        [:div]
        [:div {:style  {:color "red"}}
...
(defn register-success [res data state]
  (do
    (reset! data {:role "bookadmin" :mobilenumber ""})
    (swap! state assoc :focus false)
    (boot-notify
     "success" "Success :"
     "User successfully created")))

(defn register-error [res]
  (case (res :status)
    401 (do (swap! session dissoc :session)
            (pushy/set-token! routes/history (routes/url-for :home)))
    (boot-notify
...
;; users component
;; -------------------------------------------------------
(defn delete-user-on-click [id src-in list]
  (ajax/DELETE
   (str server "users/" @id)
   {:handler #(delete-record-handler % src-in list)
    :error-handler #(delete-record-handler % src-in list)
...
(defn delete-user-table-modal [id src-in list]
  [:div.modal.fade
   {:role "dialog" :id "removemodal"}
   [:div.modal-dialog
    [:div.modal-content
     ...
     [:div.modal-footer
      [:button.btn.btn-primary
       {:id (str @id "remove")
        :type "button"
        :on-click #(delete-user-on-click id src-in list)} "Yes"]
      [:button.btn.btn-default
       {:type "button"
        :data-dismiss "modal"} "No"]]]]])
...
(defn users-comp []
  (let [role (get-in @session [:session :user :role])
        user-id (get-in @session [:session :user :id])
        src-in (r/atom {:url "users/search"
                        :idx 1
                        :type "username"
                        :text ""})
        list (r/atom {:data []
    ...
    (fn []
      [:div.box.box-primary
       [:div.box-header.with-border
        [:i.fa.fa-users]
        [:h3.box-title "List of Registered Users"]
        (when (= role "bookadmin")
          [:div.box-tools.pull-right
           [:a.btn.btn-primary.btn-sm
            {:href (routes/url-for :add-user)}
            "Add New User"]])]
       [:div.box-body
        [delete-user-table-modal remove-id src-in list]
        [:div.row
         [:div.col-md-6]
         [:div.col-md-6
          [:div.col-md-12.col-sm-12.col-xs-12.input-group.form-group
           [search-input "usersrctext" "text"
            "Enter something to search" (r/cursor src-in [:text])
            src-in list "100%"]]]]
        [:div.table-responsive
         [:table.table.table-bordered.table-hover
             ...
             [:tr
              ...
              (when (= role "bookadmin")
                [:td [anchor-tooltip
                      {:class "btn btn-success btn-sm"
                       :title "Edit User"
                       :href (routes/url-for :edit-user :id (:id i))}
                      [:span.glyphicon.glyphicon-edit {:aria-hidden true}]]])
              [:td (:firstname i)]
              [:td (:lastname i)]
              [:td (:email i)]
       ...
       [:div.box-footer
        [pagination
         {:class "pull-right no-margin"
          ...
          :ellipsis true
          :boundaryLinks true
          :items (int (Math/ceil (/ (@list :totalusers) 10)))
          :maxButtons 5
          :activePage (@src-in :idx)
          :onSelect #(do (swap! src-in assoc :idx %)
                         (get-table-data src-in list))}]]])))
...
;; --------------------------------------------------------------
;; add or edit user comp
;; --------------------------------------------------------------
(defn post-put-register-success [res data state]
  (do (reset! data {:role "bookadmin" :mobilenumber ""})
      (swap! state assoc :focus false)
      (swap! state assoc :busy false)
      (boot-notify
       "success" "Success :"
...
(defn post-put-register-error [res state]
  (case (res :status)
    401 (do (swap! session dissoc :session)
            (pushy/set-token! routes/history (routes/url-for :home)))
    (do (swap! state assoc :busy false)
...
(defn post-put-register-onclick [data state]
  (if (nil? (register-validator @data))
    (do
      (swap! state assoc :busy true)
      (if (:id @data)
        (ajax/PUT
         (str server "register/" (:id @data))
         {:params @data
          :handler #(pushy/set-token! routes/history (routes/url-for :all-users))
          :error-handler #(post-put-register-error % state)
          :format :json
          :response-format :json
          :keywords? true
          :headers {:Authorization
        ...
        (ajax/POST
         (str server "register")
         {:params @data
          :handler #(post-put-register-success % data state)
          :error-handler #(post-put-register-error % state)
          :format :json
          :response-format :json
          :keywords? true
...
(defn add-or-edit-user-comp [data]
  (let [register-data (r/atom data)
        state (r/atom {:busy false :focus false})]
    (fn []
      [:div.box.box-primary
       [:div.box-header.with-border
        [:i.fa.fa-users]
       ...
       [:div.box-body
        [:div.form-horizontal
         [:div.row
          [:div.col-md-10
           [row-input "First Name" "firtsname" :firstname
            "text" "First Name"  (r/cursor register-data [:firstname])
            (@state :focus) (register-validator @register-data)]
           [row-input "Last Name" "LastName" :lastname
            "text" "Last Name" (r/cursor register-data [:lastname])
            (@state :focus) (register-validator @register-data)]
           (if (:id @register-data)
             [:div.form-group
              ...
              [:div.col-md-8
               [:input.form-control
                {:type "email"
                 :disabled true
                 :value (get @register-data :email "")}]]]
             [row-input "Email" "Email" :email
              "email" "Email"  (r/cursor register-data [:email])
              (@state :focus) (register-validator @register-data)])
           (when-not (:id @register-data)
             [row-input "Password" "password" :password
              "password" "Password" (r/cursor register-data [:password])
              (@state :focus) (register-validator @register-data)])
           [row-input "Mobile Number" "mobilenumber" :mobilenumber
            "text" "Mobile Number" (r/cursor register-data [:mobilenumber])
            (@state :focus) (register-validator @register-data)]
           [row-register-select "Role" :role register-options
            register-data  (@state :focus)
            (r/cursor register-data [:role])]]]]]
       [:div.box-footer.clearfix
        [:center
         [:button.btn.btn-primary
          {:type "button"
           :on-click #(post-put-register-onclick register-data state)
           :disabled (get @state :busy)}
          (if (:id @register-data)
            "Update User "
            "Register New User ")
          (when (get @state :busy)
            [:i.fa.fa-spinner.fa-pulse])] " "
         [:a.btn.btn-default
          {:href (routes/url-for :all-users)} "Back"]]]])))
...
(defn itineraries-tabs-comp [id comp]
  (let [role (get-in @session [:session :user :role])
        user-name (str (get-in @session [:session :user :firstname] "") " "
                       (get-in @session [:session :user :lastname] ""))]
    [:div.nav-tabs-custom
     [:ul.nav.nav-tabs
      ...
      [:li {:class (str "dropdown " (when (check-all-routes "/all-vendors" 12) "active"))}
       [:a.dropdown-toggle
        {:href "#",
      ...
      (when (= role "bookadmin")
        [:li (when (or (= (routes/url-for :all-users) (pushy/get-token routes/history))
                       (= (routes/url-for :add-user) (pushy/get-token routes/history))
                       (= (routes/url-for :edit-user :id id) (pushy/get-token routes/history)))
               {:class "active"})
         [:a {:href (routes/url-for :all-users)} "Registered Users"]])
      ...
      [:li.pull-right
       [:div
        ...
        [:div.btn-group
         [:span.btn.btn-default user-name]
         [:button.btn.btn-info
          {:id "Logout"
           :on-click #(do
                        (pushy/set-token!
